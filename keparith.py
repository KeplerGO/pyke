...
from numpy import absolute, float32, isfinite, maximum, mean, median, multiply, nan, ones, shape, std, subtract, where
import pyfits
import kepfit
import kepio
import kepkey
...
def kepaddconstant(infile, outfile, datacol, constant, constantval, sign, clobber, verbose, logfile, status):
    # log the call

    hashline = '----------------------------------------------------------------------------'
    kepmsg.log(logfile, hashline, verbose)
    overwrite = 'y' if clobber else 'n'
    chatter = 'y' if verbose else 'n'

    call = 'KEPARITH -- infile={} outfile={} datacol={} '.format(infile, outfile, datacol)
    call += 'constant={} constantval={} sign{} '.format(constant, constantval, sign)
    call += 'clobber={} verbose={} logfile={}\n'.format(overwrite, chatter, logfile)
    kepmsg.log(logfile, call, verbose)
    ...
    if kepio.fileexists(outfile):
        message = 'ERROR -- KEPARITH: {} exists. Use clobber=yes'.format(outfile)
...
if '--shell' in sys.argv:
    ...
    kepaddconstant(args.infile, args.outfile, args.datacol, args.constantfunc, args.constantval,
                   args.operation, args.clobber, args.verbose, args.logfile, args.status)
else:
