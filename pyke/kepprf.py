...
__all__ = ['kepprf', 'read_and_interpolate_prf']
...
def kepprf(infile, prfdir, frameno, columns, rows, fluxes, background=False,
    ...
    (splineInterpolation, DATx, DATy, prf, _, _, PRFx0, PRFy0, cdelt1p, 
        cdelt2p, prfDimX, prfDimY) = read_and_interpolate_prf(prfdir=prfdir, 
                                        module=module, output=output, 
                                        column=column, row=row, xdim=xdim, 
                                        ydim=ydim, verbose=verbose, 
                                        logfile=logfile)
