...
def test_prf_vs_aperture_photometry():
    ...
    logL.fit(x0=(fluxo, colo, rowo, 1, 1, bkgo))
    prf_flux, prf_col, prf_row, prf_stretch_col, prf_stretch_row, prf_bkg = logL.opt_result.x
    ...
    assert np.isclose(opt_params[1], col+9, rtol=1e-3)
    assert np.isclose(opt_params[2], row+9, rtol=1e-3)
